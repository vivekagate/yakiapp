...
use serde::{Serialize};
use kube::{api::{Api, ListParams, ResourceExt}, Client, Config, Error};
...
pub fn restart_deployment(window: Window, cluster: &str, arg_map: HashMap<String, String>, cmd: &str) {
  ...
  match result {
    Ok(res) => {
      let json = "success";
      window.emit("app::command_result", CommandResult{
    ...
    Err(err) => {
      ...
      window.emit("app::error", CommandResult{
        command: String::from(cmd),
        data: err.to_string()
      }).unwrap();
    }
...
#[tokio::main]
async fn _restart_deployment(cluster: &str, namespace: &str, deployment: &str) -> Result<Deployment, Box<dyn std::error::Error>> {
...
pub fn get_all_deployments(window: &Window, cluster: &str, namespace: &String, cmd: &str) -> Vec<DeploymentHolder> {
...
#[tokio::main]
async fn _populate_deployments(window: &Window, ns: &String, deploys: Vec<DeploymentHolder>) -> Result<(), Box<dyn std::error::Error>> {
...
#[tokio::main]
async fn _get_all_deployments(cluster: &str, namespace: &String) -> Result<Vec<DeploymentHolder>, Box<dyn std::error::Error>> {
  let client = init_client(cluster);
  let deploy_request: Api<Deployment> = Api::namespaced(client.await.unwrap(), namespace);
  ...
  for p in deploy_request.list(&lp).await? {
    if let Some(ref status) = p.status {
      ...
      // kdeploy.name = p.name_any();
      // kdeploy.replicas = status.replicas.unwrap_or_default();
      // kdeploy.available_replicas = status.available_replicas.unwrap_or_default();
      // kdeploy.ready_replicas = status.ready_replicas.unwrap_or_default();
      // kdeploy.unavailable_replicas = status.unavailable_replicas.unwrap_or_default();
      // kdeploy.creation_ts = p.metadata.creation_timestamp.unwrap().0.timestamp_millis();
      // if let Some(ref spec) = p.spec {
      //   if let Some(ref md) = spec.template.metadata {
      //     if let Some(ref annotations) = md.annotations {
      //       for an in annotations {
      //         let (k, v) = an;
      //         if k.ends_with("/restartedAt") {
      //           kdeploy.restart_ts = DateTime::<Utc>::from_str(v).unwrap().timestamp_millis();
      //         }
      //       }
      //     }
      //   }
      ...
      //       match lbl {
      //         (key, value) => {
      //           kdeploy.match_labels.insert(key.to_string(), value.to_string());
      //         }
      //       }
      //     }
      //   }
      // }
      deploys.push(DeploymentHolder{
        deployment: p
      });
    }
  }
...
pub fn stream_cpu_memory_for_pod(window: Window, cluster: &str, pod: &str, ns: &str, rx: &Receiver<String>) {
  _stream_cpu_memory_for_pod(window, cluster, pod, ns, rx);
}


#[tokio::main]
async fn _stream_cpu_memory_for_pod(window: Window, cluster: &str, pod: &str, ns: &str, rx: &Receiver<String>) -> Result<(), Box<dyn std::error::Error>>{
  info!("Fetching metrics for {:?}", pod);
  let client = init_client(cluster);

  let podMetrics: Api<crate::kube::metrics::PodMetrics> = Api::namespaced(client.await.unwrap(), ns);
  loop {
    ...
    let memory_string = format!("{:?}", memory).replace("Quantity(\"","").replace("\")","");
    let cpu_string = format!("{:?}", cpu).replace("Quantity(\"","").replace("\")","");
    ...
    window.emit("app::metrics", Payload { message: json, metadata: String::from(pod) }).unwrap();
...
#[tokio::main]
pub async fn get_metrics_for_deployment(window: &Window, cluster: &str, ns: &String, deployment: &str, cmd: &str) {
...
async fn _get_metrics_for_deployment(ns: &String, cluster: &str, deployment: &str) -> Result<Vec<Metric>, Box<dyn std::error::Error>>{
...
#[tokio::main]
async fn _get_logs_for_pod(window: Window, cluster: &str, pod: &str, ns: &str) -> Result<(), Box<dyn std::error::Error>> {
  info!("Fetching logs for {:?}", pod);
  let client = init_client(cluster);
  let pods: Api<Pod> = Api::namespaced(client.await.unwrap(), ns);
  let mut logs = pods.log_stream(&pod, &LogParams {
    follow: false,
    tail_lines: Some(100),
    ..LogParams::default()})
    .await?
  ...
  while let Some(line) = logs.try_next().await? {
    ...
    debug!("{:?}", line_str);
    window.emit("dashboard::logs", Payload { message: line_str.to_string(), metadata: String::from(pod) }).unwrap();
  }
...
pub fn tail_logs_for_pod(window: Window, cluster: &str, pod: &str, ns: &str, rx: &Receiver<String>) {
  _tail_logs_for_pod(window, cluster, pod, ns, rx);
}

#[tokio::main]
async fn _tail_logs_for_pod(window: Window, cluster: &str, pod: &str, ns: &str, rx: &Receiver<String>) -> Result<(), Box<dyn std::error::Error>> {
  info!("Fetching logs for {:?}", pod);
  let client = init_client(cluster);
  let pods: Api<Pod> = Api::namespaced(client.await.unwrap(), ns);
  let mut logs = pods.log_stream(&pod, &LogParams {
...
#[tokio::main]
pub async fn get_pods_for_deployment_async(window: &Window, cluster: &str, ns: &String, deployment: &str, cmd: &str) {
  let pods = _get_pods_for_deployment(ns, cluster, deployment).await;
  match pods {
    Ok(pods) => {
...
#[tokio::main]
pub async fn get_pods_for_deployment(ns: &String, cluster: &str, deployment: &str) -> Result<Vec<Pod>, Error> {
  _get_pods_for_deployment(ns, cluster, deployment).await
}

async fn _get_pods_for_deployment(ns: &String, cluster: &str, deployment: &str) -> Result<Vec<Pod>, Error> {
  let client = init_client(cluster);
