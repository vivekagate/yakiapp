...
fn init_tauri() {
  tauri::Builder::default()
    ...
    .invoke_handler(tauri::generate_handler![execute_command, execute_sync_command])
    .run(tauri::generate_context!())
    .expect("error while running tauri application");
}

#[tauri::command]
fn execute_sync_command(window: Window, commandstr: &str, appmanager: State<SingletonHolder>) -> String{
  ...
  let current_cluster =  appmanager.0.lock().unwrap().cachemanager.get(cache::KEY_CONTEXT, "");
  ...
  if cmd_hldr.command == GET_PODS_FOR_DEPLOYMENT {
    ...
    appmanager.0.lock().unwrap().cachemanager.set(cache::KEY_CONTEXT, cl);
...
fn execute_command(window: Window, commandstr: &str, appmanager: State<SingletonHolder>) {
  ...
  if cmd_hldr.command == GET_ALL_NS {
    ...
    let hndl = thread::spawn( move || {
      let namespace = cmd_hldr.args.get("ns").unwrap();
      let deployment = cmd_hldr.args.get("deployment").unwrap();
      let deploys = kube::get_pods_for_deployment_async(&window, &current_cluster, namespace, deployment, GET_PODS_FOR_DEPLOYMENT);
    });
  }   else if cmd_hldr.command == GET_METRICS_FOR_DEPLOYMENT {
    let hndl = thread::spawn( move || {
      let namespace = cmd_hldr.args.get("ns").unwrap();
      let deployment = cmd_hldr.args.get("deployment").unwrap();
      kube::get_metrics_for_deployment(&window, &current_cluster, namespace, deployment, GET_METRICS_FOR_DEPLOYMENT);
    });
    ...
    appmanager.0.lock().unwrap().taskmanager.add_metrics_stream(tx);
    ...
    let hndl = thread::spawn( move || {
      ...
      if clusters.is_empty() {
        ...
        window.emit("app_events_channel", EventHolder{
